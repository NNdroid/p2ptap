...
install_service() {
    ...
    if [ -f "$SCRIPT_DIR/p2ptap-boot" ]; then
        cp "$SCRIPT_DIR/p2ptap-boot" "$BIN_PATH"
    elif [ -f "$ROOT_DIR/bin/p2ptap-boot" ]; then
        cp "$ROOT_DIR/bin/p2ptap-boot" "$BIN_PATH"
    elif [ -f "./p2ptap-boot" ]; then
        cp "./p2ptap-boot" "$BIN_PATH"
    elif [ -f "$ROOT_DIR/cmd/p2ptap-boot/main.go" ] && command -v go >/dev/null 2>&1; then
        echo "[*] Compiling p2ptap-boot binary from source..."
        (cd "$ROOT_DIR" && CGO_ENABLED=0 go build -ldflags="-s -w $VER_FLAGS" -o "$BIN_PATH" ./cmd/p2ptap-boot)
    else
        echo "[!] Error: p2ptap-boot binary or Go build environment not found!"
        exit 1
    fi
...
update_service() {
    ...
    if [ -f "$SCRIPT_DIR/p2ptap-boot" ]; then
        ...
        echo "[!] Error: Updated p2ptap-boot binary or Go build environment not found!"
        exit 1
    fi
    chmod +x "$BIN_PATH"
    systemctl restart p2ptap-boot.service
