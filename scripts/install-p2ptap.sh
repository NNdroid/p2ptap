...
install_service() {
    ...
    if [ -f "$ROOT_DIR/cmd/p2ptap/main.go" ]; then
        echo "[*] Compiling p2ptap binary..."
        (cd "$ROOT_DIR" && CGO_ENABLED=0 go build -ldflags="-s -w $VER_FLAGS" -o "$BIN_PATH" ./cmd/p2ptap)
    elif [ -f "$SCRIPT_DIR/p2ptap" ]; then
        cp "$SCRIPT_DIR/p2ptap" "$BIN_PATH"
    elif [ -f "./p2ptap" ]; then
        cp "./p2ptap" "$BIN_PATH"
    else
        echo "[!] Error: p2ptap binary or source code not found!"
        exit 1
    fi
...
update_service() {
    ...
    if [ -f "$ROOT_DIR/cmd/p2ptap/main.go" ]; then
        ...
        echo "[!] Error: Updated p2ptap binary not found!"
        exit 1
    fi
    chmod +x "$BIN_PATH"
    systemctl restart p2ptap.service
