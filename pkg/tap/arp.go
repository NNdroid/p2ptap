...
func buildARPFrame(localMAC, dstMAC net.HardwareAddr, arpPayload []byte) []byte {
	...
	if len(localMAC) == 6 {
		...
		copy(frame[6:12], []byte{0x02, 0x00, 0x5e, 0x10, 0x00, 0x01})
	}
...
func BuildARPReplyFrame(localMAC, senderMAC net.HardwareAddr, targetIP, senderIP net.IP) []byte {
	...
	if len(localMAC) == 6 {
		...
		copy(reply[6:12], []byte{0x02, 0x00, 0x5e, 0x10, 0x00, 0x01})
	}
...
func ShouldRespondToARP(targetIP, localIP, webUIVirtualIP net.IP, localNetwork *net.IPNet) bool {
	...
	// Respond if the target is within our local subnet (e.g., for proxy ARP)
	if localNetwork != nil && localNetwork.Contains(targetIP4) {
		return true
	}
...
func BuildIPv6NeighborAdvertisementFrame(targetMAC net.HardwareAddr, targetIPv6 net.IP, dstIPv6 ...net.IP) []byte {
	...
	dstIP := net.ParseIP("ff02::1").To16()
	flags := byte(0x20) // Override
	if len(dstIPv6) > 0 && dstIPv6[0] != nil && len(dstIPv6[0].To16()) == 16 {
