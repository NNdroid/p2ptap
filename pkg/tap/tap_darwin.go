...
func (d *DarwinTAPDevice) MAC() string {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.mac
}
...
// SelfTest verifies the macOS TAP write/read path.
//
// macOS utun devices do NOT echo written frames back to the read queue
// (same as Linux TAP). Writes enter the kernel stack; reads return only
// frames transmitted by the kernel. Treat like Wintun: validate write+read
// callable; a read timeout is normal and healthy.
func (d *DarwinTAPDevice) SelfTest() map[string]interface{} {
	return runRealDeviceSelfTest(d, "wintun")
}
