...
// Returns true if the frame is ALLOWED (accept), or false if the frame is DENIED (drop).
func MatchACL(aclCfg *config.ACLConfig, frame []byte, peerID string, isTx bool) bool {
	if aclCfg == nil || !aclCfg.Enable {
		return true // Default: open mesh, allow all traffic
	}
