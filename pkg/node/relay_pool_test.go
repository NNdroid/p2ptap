...
func TestRelayPoolWriteFrame_Success(t *testing.T) {
	t.Log("[relay-pool] writeFrame with healthy stream should succeed and call onSent")
	rc := &relayConn{}
	...
	t.Logf("[relay-pool] ✓ wrote %d bytes, onSent fired=%v", stream.buf.Len(), sentCalled)
}

func TestRelayPoolWriteFrame_NoStream(t *testing.T) {
	t.Log("[relay-pool] writeFrame with nil stream should return errStreamNotReady")
...
func TestRelayPoolWriteFrame_WriteError(t *testing.T) {
	t.Log("[relay-pool] writeFrame with failing stream should surface the write error")
	rc := &relayConn{}
	...
	if err == nil {
		...
		t.Logf("[relay-pool] ✓ write error propagated: %v", err)
	}
}

func TestRelayPoolDrainAll(t *testing.T) {
	t.Log("[relay-pool] drainAll should invoke onFail for every queued job")
	rc := &relayConn{
	...
	if failCount != 5 {
		...
		t.Logf("[relay-pool] ✓ drainAll fired onFail for all %d jobs", failCount)
	}
...
func TestRelayPoolDrainAll_Empty(t *testing.T) {
	t.Log("[relay-pool] drainAll on empty pool should be a no-op")
	rc := &relayConn{
		writeCh: make(chan relayJob, 8),
	}
...
func TestRelayPoolCloseStream(t *testing.T) {
	t.Log("[relay-pool] closeStream should nil out the underlying stream")
	rc := &relayConn{}
	stream := &mockWriteStream{}
	rc.stream = stream
	rc.closeStream()
...
func TestRelayPoolCloseStream_Nil(t *testing.T) {
	t.Log("[relay-pool] closeStream on nil stream should not panic")
	rc := &relayConn{} // stream = nil
	// Should not panic
	rc.closeStream()
}
...
func TestRelayPoolWriteFrame_Deadline(t *testing.T) {
	t.Log("[relay-pool] writeFrame should set a non-zero write deadline")
	rc := &relayConn{}
	stream := &mockWriteStream{}
	rc.stream = stream
	...
	if stream.deadline.IsZero() {
		...
		t.Logf("[relay-pool] ✓ write deadline set: %v", stream.deadline)
	}
}
...
func TestRelayPoolWriteFrame_NoDoubleOnSent(t *testing.T) {
	t.Log("[relay-pool] onSent must fire exactly once per successful write")
	rc := &relayConn{}
	stream := &mockWriteStream{}
	rc.stream = stream
	...
	if count != 3 {
		...
		t.Logf("[relay-pool] ✓ onSent fired %d times for 3 writes", count)
	}
}
