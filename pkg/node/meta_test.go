...
func TestGatewayIPSelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Logf("[gw-select] subnet=%s tapV4=%s tapV6=%s expectV4=%v expectGW=%s",
				tt.subnet, tt.tapIPv4, tt.tapIPv6, tt.expectV4, tt.expectGW)
			...
			if gw != tt.expectGW {
				...
				t.Logf("[gw-select] ✓ selected gateway %s for %s", gw, tt.subnet)
			}
		})
	}
}
...
func TestProcessSubnetRoutesIntegration(t *testing.T) {
	...
	node, err := NewNodeWithTAP(cfg, tapDev, nil)
	if err != nil {
	...
	t.Logf("[subnet] Case1 mixed subnets %v processed without panic", subnets)
	...
	t.Logf("[subnet] Case2 empty list no-op; Case3 invalid CIDR skip; Case4 missing v6 gw warn; Case5 missing v4 gw warn; Case6 accept=false skip")
	_ = tapPipe
}
...
func TestProcessSubnetRoutesPeerAuthorization(t *testing.T) {
	...
	t.Logf("[subnet-auth] self peer %s NOT in allowlist %v -> install skipped", node.Host.ID().ShortString(), cfg.AllowedSubnetPeers)
