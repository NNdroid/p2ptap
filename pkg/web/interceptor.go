...
// It is defined once in the observer package and re-exported here so existing
// references in this file keep compiling.
var InterceptorMAC = observer.InterceptorMAC

// PacketWriter mirrors observer.PacketWriter so TAPInterceptor satisfies the
// observer.FrameFilter interface without importing node.
type PacketWriter = observer.PacketWriter
...
func (it *TAPInterceptor) MatchAndHandle(frame []byte, writer PacketWriter) bool {
	...
	etherType := packet.EtherType(frame)

	// 1. Fast-Path: IPv4 Check (EtherType == packet.EtherTypeIPv4)
	if it.enableV4 && etherType == packet.EtherTypeIPv4 {
	...
	// 2. Fast-Path: ARP Check (EtherType == packet.EtherTypeARP)
	if it.enableV4 && etherType == packet.EtherTypeARP {
	...
	// 3. Fast-Path: IPv6 Check (EtherType == packet.EtherTypeIPv6)
	if it.enableV6 && etherType == packet.EtherTypeIPv6 && len(frame) >= 54 {
		nextHeader := frame[20]
...
func (it *TAPInterceptor) handleARP(frame []byte, writer PacketWriter) bool {
	...
	binary.BigEndian.PutUint16(reply[12:14], packet.EtherTypeARP)

	binary.BigEndian.PutUint16(reply[14:16], 1)
	binary.BigEndian.PutUint16(reply[16:18], packet.EtherTypeIPv4)
	reply[18] = 6
	reply[19] = 4
...
func (it *TAPInterceptor) handleICMPv6NDP(frame []byte, writer PacketWriter) bool {
	...
	copy(reply[0:6], senderMAC)
	copy(reply[6:12], InterceptorMAC)
	binary.BigEndian.PutUint16(reply[12:14], packet.EtherTypeIPv6)
