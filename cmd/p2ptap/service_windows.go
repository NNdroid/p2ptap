...
// serviceStopCloseTimeout bounds how long the service waits for node.Close() on
// a Stop/Shutdown request before giving up. node.Close internally caps each
// teardown step at 8s (node.go Close) and runs several sequentially, so a clean
// stop can legitimately take tens of seconds on Windows (slow host/tap drain).
// This must be generously larger than the old hard-coded 3s — truncating there
// let the process exit while cleanup was still running and strand the TAP
// device / exit-node NAT. SCM tolerates StopPending this long.
const serviceStopCloseTimeout = 30 * time.Second
...
func runDaemonService() {
	...
	if logFile, err := os.OpenFile(logPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644); err == nil {
		os.Stdout = logFile
		os.Stderr = logFile
		// We are logging to a file, not a console: strip the ANSI color codes the
		// logger otherwise embeds on every line, or the log file fills with
		// "\033[32m" noise that is illegible in any viewer.
...
func (s *p2ptapDaemonService) Execute(_ []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	for c := range r {
		...
		case svc.Stop, svc.Shutdown:
			...
			// node.Close is internally bounded (each teardown step is capped at
			// 8s, see node.go Close), so it cannot hang forever — but the steps
			// run sequentially and a slow host/tap drain can exceed the old 3s
			// window. Reporting Stopped at 3s while Close still runs in the
			// background made the process exit mid-cleanup, stranding the TAP
			// device / exit-node NAT. Wait for Close to actually finish, with an
			// outer cap that comfortably covers the node's worst case; only if it
			// still exceeds do we log and let SCM move on.
			...
			select {
			case <-done:
			case <-time.After(serviceStopCloseTimeout):
				logger.New("Service").Error("node.Close() exceeded %s on service stop; forcing stop (TAP/NAT cleanup may be incomplete)",
					serviceStopCloseTimeout)
			}
